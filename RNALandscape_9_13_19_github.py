...
# import networkx; iso = networkx.isomorphism
import matplotlib.pyplot as plt #for plotting
#import tensorflow as tf #For sparse tensors -- should use the sparse package instead
...
class RNALandscape:
    ...
    def __init__(self, sequences, DNA = [False,False], sequenceConcentrations = [1,1], b = 0.8/0.33, T = 310.15, 
                  ...
                  considerC3andC4 = True,  unboundButCouldBindPenalties = [0,0]):
        
        self.version = '9_13_19' #don't forget to change this when we update the code
...
#     will be run. Still hasn't been implemented.
# 
...
# =============================================================================                
        sequences, DNA, sequenceConcentrations = self.basicCorrectionsToInput(sequences, DNA, sequenceConcentrations) 
...
#            If we have a mix of U's and T's replace them based on whether it's DNA or RNA
        ...
        return(sequences, DNA, sequenceConcentrations)
        ...
        for whichStruct in range(numStructures):
            ...
            (bondFECounts[whichStruct], dangling5Count[whichStruct], dangling3Count[whichStruct],
             specialLoopEnergies, specialLoopEntropies,unboundButCouldBindCounts[whichStruct]) = \
                 ...
                 #It's important to put the bond energy calculation first since
...
#                        since that'll be taken care of by danglingMatrixIndices
    ...
    def enumerateStructures(self, C3, C4):
# =============================================================================
...
#                                    Iterate over all triplets in current structure
        ...
        if minNumStemsInStructure <= 1:
            for i in range(numStems): #each stem can be its own helipoint
                if linkedStems[i] or not onlyConsiderBondedStrands:
                    structures.append([i])
                    numStructures += 1
...
#               duplexEntropy penalty, pairwiseEnergies, 
...
# =============================================================================
        ...
        fileName = folderName + self.fileNameFxn(self.numSequences, str(self.pairwiseEnergies), 
                ...
                str(self.duplexEntropyPenaltyInKB), 
    ...
    def fileNameFxn(self, numSequences, strPairwiseEnergies, strStoreGraphs, strT, 
                    strVs, strG, strDuplexEntropyPenaltyInKB,
                    strIncludeTerminalAUATPenalties, strIncludeDanglingEnds,
...
#    Make the backbone edges
    for i in range(numNt - 1):
        if i not in linkerPos and i+1 not in linkerPos:
            G.add_edge(nodelist[i],nodelist[i+1], backbone = True, weight = 1.2) #higher weight means more distance between bonded nodes
    
#    Make the bond edges
    for stem in structure:
        numBonds = int(len(stem)/2)
        for i in range(numBonds):
            G.add_edge(nodelist[stem[i]], nodelist[stem[i+numBonds]], weight = 1.3, backbone = False)
...
#Example code:
#q = RNALandscape(['AUCUGAUACUGUGCUAUGUCUGAGAUAGC'],storeGraphs=False, tryToLoadVariables = True) #define the object with the parameters you want
#q.mainLandscapeCalculation() #calculate the FE Lanscape
